...
class RandomGame():
    ...
    def generate_map(self):
        sheet = random.randint(0, 5)
        self.map = pd.read_excel(self.maps, sheet_name=sheet, header=None)
        for y in range(len(self.map)):
            ...
            for x in range(len(self.map[0])):
                if self.map[x][y] == 1:
                    ...
                    self.car = Car(x*self.size, y*self.size, self.size)
    ...
    def button_tracking(self, model): #model
